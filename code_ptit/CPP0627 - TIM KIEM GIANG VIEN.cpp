// CPP0627 - TIM KIEM GIANG VIEN.cpp
#include <bits/stdc++.h>
#include <iostream>
#include <algorithm>
#include <cmath>
...
private:
	string mgv, ht, bm, mm, ten, htHoa;
public:
	GiangVien() { mgv = ht = bm = " "; }
	string gethtHoa() { return this->htHoa; }
	friend istream& operator>>(istream& input, GiangVien& gv) {
		...
		gv.htHoa = gv.ht;
		transform(all(gv.htHoa), gv.htHoa.begin(), ::toupper);
		return input;
	}
...
void solve()
{
	...
	int Q; cin >> Q;
	f(i, 0, Q) {
		string tuKhoa; gl(cin, tuKhoa);
		cout << "DANH SACH GIANG VIEN THEO TU KHOA " << tuKhoa << ":\n";
		transform(all(tuKhoa), tuKhoa.begin(), ::toupper);
		for(auto gv:ds) {
			string ht = gv.gethtHoa();
			if(ht.find(tuKhoa) != string::npos) cout << gv << ln;
		}
	}
}
