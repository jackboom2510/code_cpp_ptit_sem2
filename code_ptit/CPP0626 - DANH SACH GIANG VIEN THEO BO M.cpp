// CPP0626 - DANH SACH GIANG VIEN THEO BO MON.cpp
#include <bits/stdc++.h>
#include <iostream>
#include <algorithm>
#include <cmath>
...
void solve()
{
	...
	int Q; cin >> Q;
	f(i, 0, Q) {
		string bm_; gl(cin, bm_);
		stringstream ss(bm_);
		string mm_, word;
		while(ss >> word) {
			mm_ += toupper(word[0]);
		}
		cout << "DANH SACH GIANG VIEN BO MON " << mm_ << ":\n";
		for(auto gv:ds) {
			if(gv.getMM() == mm_) cout << gv << ln;
		}
	}
}
